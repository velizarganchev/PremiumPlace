...
describe('BookingCheckoutComponent', () => {
  ...
  it('creates a pending booking then renders PayPal buttons', fakeAsync(() => {
  ...
  it('builds the order with the stay total', fakeAsync(() => {
    fixture.detectChanges();
    flushMicrotasks();

    const opts = buttonsSpy.calls.mostRecent().args[0];
    const create = jasmine.createSpy('create');
    opts.createOrder(null, { order: { create } });
  ...
  it('shows an error when the pending booking fails', fakeAsync(() => {
